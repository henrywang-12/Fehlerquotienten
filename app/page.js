...
export default function Page() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      setLines(data.lines || []);
      setTranscription(data.transcription || (data.lines || []).join("\n"));
  ...
  const reset = () => { setStep(1); setImgSrc(null); setImgData(null); setImgMime(null); setTranscription(""); setLines([]); setErrors([]); setSummary(""); setError(null); };
      ...
      <main style={s.main}>
        ...
        {step === 2 && (
          <div>
            ...
            {/* Annotated image with error overlays */}
            {imgSrc && (
              <div style={{ ...s.card, padding: 0, overflow: "hidden", marginBottom: 16 }}>
                <div style={{ padding: "10px 16px", borderBottom: "1px solid #eeede8", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <span style={{ fontSize: 11, fontWeight: 500, color: "#999", textTransform: "uppercase", letterSpacing: "0.5px" }}>{lang === "de" ? "Markiertes Arbeitsblatt" : "Annotated worksheet"}</span>
                  <div style={{ display: "flex", gap: 4 }}>
                    <button onClick={() => setViewMode("image")} style={{ fontSize: 11, padding: "3px 10px", borderRadius: 4, border: viewMode === "image" ? "1px solid #1a1a18" : "1px solid #e5e4df", background: viewMode === "image" ? "#1a1a18" : "#fff", color: viewMode === "image" ? "#fff" : "#888", cursor: "pointer", fontWeight: 500 }}>{lang === "de" ? "Bild" : "Image"}</button>
                    <button onClick={() => setViewMode("text")} style={{ fontSize: 11, padding: "3px 10px", borderRadius: 4, border: viewMode === "text" ? "1px solid #1a1a18" : "1px solid #e5e4df", background: viewMode === "text" ? "#1a1a18" : "#fff", color: viewMode === "text" ? "#fff" : "#888", cursor: "pointer", fontWeight: 500 }}>{lang === "de" ? "Text" : "Text"}</button>
                  </div>
                </div>
...
function AnnotatedImage({ imgSrc, lines, errors, highlightId, onHover }) {
  ...
  // Estimate error position on the image based on line number and word position
  const getOverlayStyle = (err) => {
    const lineIdx = typeof err.line === "number" ? err.line : 0;
    const wordStart = typeof err.wordStart === "number" ? err.wordStart : 0;
    const wordEnd = typeof err.wordEnd === "number" ? err.wordEnd : wordStart + 1;
    const lineText = lines[lineIdx] || "";
    const wordsInLine = lineText.split(/\s+/).filter(Boolean);
    const totalWords = wordsInLine.length || 1;

    // Vertical: distribute lines evenly across ~15%-85% of image height (text area)
    ...
    const top = topMargin + lineIdx * lineHeight;

    // Horizontal: estimate word position within line
    const leftMargin = 0.06;
    const rightMargin = 0.06;
    const usableWidth = 1 - leftMargin - rightMargin;
    const charsBefore = wordsInLine.slice(0, wordStart).join(" ").length;
    const charsError = wordsInLine.slice(wordStart, wordEnd).join(" ").length;
    const totalChars = lineText.length || 1;
    const left = leftMargin + (charsBefore / totalChars) * usableWidth;
    const width = Math.max(0.04, (charsError / totalChars) * usableWidth);
  ...
  return (
    <div ref={containerRef} style={{ position: "relative", background: "#f9f9f6", padding: 0, lineHeight: 0 }}>
      <img
        src={imgSrc}
        ...
        alt=""
      />
      {/* Error overlays on the image */}
      {imgSize.w > 0 && activeErrors.map((err) => {
        const col = ECOLORS[err.type] || ECOLORS.G;
        const isHovered = highlightId === err.id;
        const pos = getOverlayStyle(err);

        return (
          <div
            key={err.id}
            onMouseEnter={() => onHover(err.id)}
            onMouseLeave={() => onHover(null)}
            style={{
              ...pos,
              background: isHovered ? col.border + "40" : col.border + "25",
              borderBottom: `3px solid ${col.border}`,
              borderRadius: 3,
              cursor: "pointer",
              transition: "background 0.15s",
              display: "flex",
          ...
          >
            ...
            {isHovered && (
              <div style={{
                ...
                background: "#fff", border: `1px solid ${col.border}`, borderRadius: 6,
                padding: "6px 10px", minWidth: 160, maxWidth: 260,
                boxShadow: "0 4px 12px rgba(0,0,0,0.15)", zIndex: 20,
                lineHeight: "normal", fontSize: 12,
              }}>
                <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 3 }}>
                  <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 5px", borderRadius: 3, background: col.badge, color: col.text }}>{err.type}</span>
                  <span style={{ fontWeight: 600, color: "#1a1a18" }}>{err.original}</span>
                  <span style={{ color: "#999" }}>&rarr;</span>
                  <span style={{ color: "#1a7a3a", fontStyle: "italic" }}>{err.correction}</span>
                </div>
                <div style={{ fontSize: 11, color: "#888" }}>{err.explanation}</div>
              </div>
            )}
          </div>
        );
