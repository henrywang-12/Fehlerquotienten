...
const T = {
  de: {
    ...
    uploadTitle: "Klausur hochladen", uploadDesc: "Mehrere Seiten? Einfach alle Bilder auswählen",
    takePhoto: "Foto aufnehmen", uploadFile: "Dateien auswählen",
    ...
    pages: "Seiten", addMore: "Weitere Seiten hinzufügen", page: "Seite",
    originalImages: "Originalbilder",
  },
  en: {
    title: "Error Quotient", subtitle: "AI-powered exam analysis",
    uploadTitle: "Upload exam", uploadDesc: "Multiple pages? Just select all images at once",
    takePhoto: "Take photo", uploadFile: "Choose files",
    ...
    pages: "Pages", addMore: "Add more pages", page: "Page",
...
export default function Page() {
  ...
  const handleFiles = useCallback(async (fileList) => {
    ...
    const newImgs = await Promise.all(files.map(async (file) => {
      ...
      return { src: compressed, data: compressed.split(",")[1], mime: "image/jpeg", name: file.name };
    }));
  ...
  const analyze = async () => {
    ...
    try {
      const resp = await fetch("/api/analyze", {
        ...
        body: JSON.stringify({ images: images.map(img => ({ data: img.data, mime: img.mime })), lang, scanLang }),
      });
      const text = await resp.text();
      let data;
      try { data = JSON.parse(text); } catch { throw new Error(lang === "de" ? "Server-Antwort konnte nicht verarbeitet werden." : "Could not process server response."); }
  ...
  const addErr = (err) => { setErrors((p) => [...p, { ...err, id: Date.now(), status: "manual" }]); setShowAdd(false); };

  const printContent = () => {
    const el = ELABELS[lang];
    const rows = active.map((e) => `<tr><td style="padding:6px 10px;border-bottom:1px solid #eee;font-weight:600;font-size:12px;color:${ECOLORS[e.type].text};background:${ECOLORS[e.type].bg}">${el[e.type]}</td><td style="padding:6px 10px;border-bottom:1px solid #eee;font-size:13px"><strong>${e.original}</strong></td><td style="padding:6px 10px;border-bottom:1px solid #eee;font-size:13px">${e.correction}</td><td style="padding:6px 10px;border-bottom:1px solid #eee;font-size:12px;color:#666">${e.explanation}</td></tr>`).join("");
  ...
  const reset = () => { setStep(1); setImages([]); setTranscription(""); setErrors([]); setSummary(""); setError(null); };
  ...
  const S = {
    ...
    primary: { background: "#1a1a18", color: "#fff" },
    secondary: { background: "#fff", color: "#666", border: "1px solid #e5e4df" },
    disabled: { background: "#e5e4df", color: "#bbb", cursor: "not-allowed" },
    sel: { fontSize: 12, border: "1px solid #e5e4df", borderRadius: 6, padding: "6px 8px", background: "#fff", color: "#1a1a18" },
    lbl: { fontSize: 10, fontWeight: 500, color: "#999", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 8 },
    ov: { position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 16 },
    mo: { background: "#fff", borderRadius: 12, padding: 24, maxWidth: 420, width: "100%" },
  };
    ...
    <div style={S.page}>
      <header style={S.header}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <select value={lang} onChange={(e) => setLang(e.target.value)} style={S.sel}><option value="de">DE</option><option value="en">EN</option></select>
        </div>
      </header>
      ...
      <main style={S.main}>
        ...
        {step === 1 && (
          <div>
            <div
              style={{ background: "#fff", border: images.length ? "2px solid #1a7a3a" : "2px dashed #ddd", borderRadius: 12, padding: images.length ? 16 : "40px 16px", textAlign: "center", cursor: images.length ? "default" : "pointer", marginBottom: 12 }}
              ...
              {images.length === 0 ? (
                <div>
                  ...
                  <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
                    <button onClick={(e) => { e.stopPropagation(); camRef.current?.click(); }} style={{ ...S.btn, ...S.secondary, padding: "8px 16px", fontSize: 12, fontWeight: 500 }}>{t.takePhoto}</button>
                    <button onClick={(e) => { e.stopPropagation(); fileRef.current?.click(); }} style={{ ...S.btn, ...S.primary, padding: "8px 16px", fontSize: 12 }}>{t.uploadFile}</button>
                  </div>
                </div>
              ) : (
                <div>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 10, marginBottom: 12 }}>
                    {images.map((img, idx) => (
                      <div key={idx} style={{ position: "relative", borderRadius: 8, overflow: "hidden", border: "1px solid #eee" }}>
                        <img src={img.src} style={{ width: "100%", height: 120, objectFit: "cover", display: "block" }} alt="" />
                        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "rgba(0,0,0,0.55)", padding: "4px 8px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                          <span style={{ fontSize: 11, color: "#fff", fontWeight: 500 }}>{t.page} {idx + 1}</span>
                          <div style={{ display: "flex", gap: 3 }}>
                            {idx > 0 && <button onClick={(e) => { e.stopPropagation(); moveImage(idx, -1); }} style={{ fontSize: 10, background: "rgba(255,255,255,0.2)", border: "none", color: "#fff", borderRadius: 3, padding: "1px 5px", cursor: "pointer" }}>↑</button>}
                            {idx < images.length - 1 && <button onClick={(e) => { e.stopPropagation(); moveImage(idx, 1); }} style={{ fontSize: 10, background: "rgba(255,255,255,0.2)", border: "none", color: "#fff", borderRadius: 3, padding: "1px 5px", cursor: "pointer" }}>↓</button>}
                            <button onClick={(e) => { e.stopPropagation(); removeImage(idx); }} style={{ fontSize: 10, background: "rgba(200,50,50,0.8)", border: "none", color: "#fff", borderRadius: 3, padding: "1px 5px", cursor: "pointer" }}>✕</button>
                          </div>
                        </div>
                      </div>
                    ))}
                  </div>
                  <button onClick={(e) => { e.stopPropagation(); addMoreRef.current?.click(); }} style={{ fontSize: 12, color: "#666", background: "#f5f5f2", border: "1px dashed #ccc", borderRadius: 6, padding: "6px 16px", cursor: "pointer" }}>+ {t.addMore}</button>
                  <div style={{ fontSize: 11, color: "#999", marginTop: 6 }}>{images.length} {t.pages}</div>
                </div>
              )}
            </div>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 4, alignItems: "center" }}>
              ...
              <select value={scanLang} onChange={(e) => setScanLang(e.target.value)} style={S.sel}>
                <option>{t.deutsch}</option><option>{t.englisch}</option>
              </select>
              <div style={{ flex: 1 }} />
              <button onClick={analyze} disabled={!images.length || loading} style={{ ...S.btn, ...(images.length && !loading ? S.primary : S.disabled) }}>
          ...
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, marginBottom: 16 }}>
              {[{ l: t.words, v: wc }, { l: t.errors, v: active.length }, { l: t.fq, v: fq, c: gr.c }, { l: t.grade, v: gr.label, c: gr.c }].map((m, i) => (
                <div key={i} style={S.card}><div style={{ fontSize: 10, color: "#999", marginBottom: 2 }}>{m.l}</div><div style={{ fontSize: 20, fontWeight: 600, color: m.c || "#1a1a18" }}>{m.v}</div></div>
              ))}
            </div>

            <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
              {Object.entries(tc).map(([type, count]) => (
                <span key={type} style={{ fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 20, background: ECOLORS[type].bg, color: ECOLORS[type].text, border: `1px solid ${ECOLORS[type].border}20` }}>{ELABELS[lang][type]}: {count}</span>
              ))}
            </div>
            ...
            {images.length > 0 && (
              <div style={{ ...S.card, padding: 0, overflow: "hidden", marginBottom: 16 }}>
                <button onClick={() => setShowImages(!showImages)} style={{ width: "100%", padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", background: "none", border: "none", borderBottom: showImages ? "1px solid #eeede8" : "none", cursor: "pointer", fontSize: 12, fontWeight: 500, color: "#666" }}>
                  <span>{t.originalImages} ({images.length} {t.pages})</span>
                  <span style={{ fontSize: 14, display: "inline-block", transform: showImages ? "rotate(180deg)" : "rotate(0)", transition: "transform 0.2s" }}>&#9662;</span>
                </button>
                {showImages && (
                  <div style={{ padding: 12, background: "#f9f9f6", display: "flex", gap: 10, overflowX: "auto" }}>
                    {images.map((img, idx) => (
                      <div key={idx} style={{ flexShrink: 0, textAlign: "center" }}>
                        <div style={{ fontSize: 10, color: "#999", marginBottom: 4 }}>{t.page} {idx + 1}</div>
                        <img src={img.src} style={{ height: 220, borderRadius: 6, border: "1px solid #eee", display: "block" }} alt="" />
                      </div>
                    ))}
                  </div>
                )}
            ...
            {transcription && (
              <div style={{ ...S.card, marginBottom: 16 }}>
                <div style={{ ...S.lbl, marginBottom: 10 }}>{t.recognized}</div>
                <div style={{ fontSize: 14, lineHeight: 2, color: "#1a1a18" }}>
                  <HighlightedText text={transcription} errors={errors} highlightId={highlightId} onHover={setHighlightId} lang={lang} />
                </div>
              </div>
            )}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <span style={S.lbl}>{t.errorList}</span>
            ...
            {errors.map((err) => (
              <div key={err.id} onMouseEnter={() => setHighlightId(err.id)} onMouseLeave={() => setHighlightId(null)}
                style={{ background: err.status === "rejected" ? "#fafaf8" : highlightId === err.id ? "#f8f8f5" : "#fff", border: "1px solid #eeede8", borderLeft: `3px solid ${err.status === "rejected" ? "#ddd" : ECOLORS[err.type]?.border || "#ddd"}`, borderRadius: "0 8px 8px 0", padding: "12px 14px", marginBottom: 8, opacity: err.status === "rejected" ? 0.4 : 1, transition: "all 0.15s" }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4, flexWrap: "wrap" }}>
                      ...
                      {err.status !== "pending" && <span style={{ fontSize: 9, padding: "1px 6px", borderRadius: 3, background: err.status === "rejected" ? "#f5f5f2" : err.status === "manual" ? "#E6F1FB" : "#eef7f0", color: err.status === "rejected" ? "#999" : err.status === "manual" ? "#0C447C" : "#1a7a3a" }}>{t[err.status]}</span>}
                    </div>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 20 }}>
              <button onClick={() => setStep(1)} style={{ ...S.btn, ...S.secondary, flex: 1, padding: 10, fontSize: 12 }}>{t.back}</button>
              <button onClick={() => setStep(3)} style={{ ...S.btn, ...S.primary, flex: 2, padding: 10, fontSize: 13 }}>{t.step3} &rarr;</button>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
              <button onClick={doDownload} style={{ ...S.btn, ...S.secondary, flex: 1, padding: 12, fontSize: 13 }}>{t.download}</button>
              <button onClick={doPrint} style={{ ...S.btn, ...S.primary, flex: 1, padding: 12, fontSize: 13 }}>{t.print}</button>
            </div>
            <button onClick={reset} style={{ ...S.btn, ...S.secondary, width: "100%", padding: 10, fontSize: 12, color: "#888" }}>{t.newAnalysis}</button>
          </div>
        )}
      </main>

      {showAdd && <div style={S.ov} onClick={() => setShowAdd(false)}><div style={S.mo} onClick={(e) => e.stopPropagation()}><AddModal lang={lang} t={t} onSave={addErr} onClose={() => setShowAdd(false)} /></div></div>}
      {showPrivacy && <div style={S.ov} onClick={() => setShowPrivacy(false)}><div style={S.mo} onClick={(e) => e.stopPropagation()}><h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12 }}>{t.privacy}</h3><p style={{ fontSize: 13, color: "#555", lineHeight: 1.7, marginBottom: 16 }}>{t.privacyText}</p><button onClick={() => setShowPrivacy(false)} style={{ width: "100%", padding: 8, fontSize: 12, border: "1px solid #e5e4df", borderRadius: 6, background: "#fff", cursor: "pointer" }}>{t.close}</button></div></div>}
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
        {["G","R","W","Z"].map((tp) => (<button key={tp} onClick={() => setType(tp)} style={{ flex: 1, padding: 6, fontSize: 11, fontWeight: 600, borderRadius: 6, cursor: "pointer", border: type===tp ? `2px solid ${ECOLORS[tp].border}` : "1px solid #e5e4df", background: type===tp ? ECOLORS[tp].bg : "#fff", color: type===tp ? ECOLORS[tp].text : "#888" }}>{ELABELS[lang][tp]}</button>))}
      </div>
...
function HighlightedText({ text, errors, highlightId, onHover, lang }) {
  ...
  activeErrors.forEach((err) => {
    ...
    let i = 0;
    while (true) {
      const idx = lower.indexOf(orig, i);
      if (idx === -1) break;
      if (!matches.find((m) => m.id === err.id)) matches.push({ start: idx, end: idx + err.original.length, err });
      i = idx + 1;
    }
  });
  ...
  matches.forEach((m) => {
    ...
    segs.push({ type: "error", content: text.slice(m.start, m.end), err: m.err });
    last = m.end;
  });
  ...
  return (
    <span>
      {segs.map((seg, i) => {
        ...
        const hov = highlightId === e.id;
        return (
          <span key={i} onMouseEnter={() => onHover(e.id)} onMouseLeave={() => onHover(null)}
            style={{ background: hov ? col.border + "30" : col.bg, borderBottom: `2px solid ${col.border}`, borderRadius: 2, padding: "1px 2px", cursor: "pointer", transition: "background 0.15s" }}
            title={`${ELABELS[lang][e.type]}: ${e.original} → ${e.correction}`}>
            {seg.content}
            <span style={{ fontSize: 8, fontWeight: 700, padding: "0 3px", borderRadius: 3, background: col.border, color: "#fff", marginLeft: 2, verticalAlign: "super", lineHeight: 1 }}>{e.type}</span>
          </span>
        );
      })}
    </span>
